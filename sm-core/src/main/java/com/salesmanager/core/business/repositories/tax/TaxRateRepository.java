...
public interface TaxRateRepository extends JpaRepository<TaxRate, Long> {
	...
	@Query("select t from TaxRate t join fetch t.taxClass ttc join fetch t.merchantStore tm join fetch t.country tc left join fetch t.zone tz left join fetch t.descriptions td left join t.parent where tm.id=?1 AND (tz.id=?2 OR tz IS NULL) and tc.id=?3 and td.language.id=?4 and ttc.id=?5 order by t.taxPriority asc")
	List<TaxRate> findByMerchantAndZoneAndCountryAndLanguageAndTaxClass(Integer id, Long zoneId, Integer countryId, Integer languageId, Long taxClassId);

	@Query("select t from TaxRate t join fetch t.taxClass ttc join fetch t.merchantStore tm join fetch t.country tc left join fetch t.zone tz left join fetch t.descriptions td left join t.parent where tm.id=?1 AND t.stateProvince=?2 and tc.id=?3 and td.language.id=?4 and ttc.id=?5 order by t.taxPriority asc")
	List<TaxRate> findByMerchantAndProvinceAndCountryAndLanguageAndTaxClass(Integer id, String province, Integer countryId, Integer languageId, Long taxClassId);
}
