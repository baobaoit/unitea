...
public class UserServiceImpl extends SalesManagerEntityServiceImpl<Long, User> implements UserService {
	...
	@Override
	public List<User> listByGroupsAndStore(List<String> groups, MerchantStore store) throws ServiceException {
		Validate.notNull(groups, "Groups cannot be null");
		Validate.notNull(store, "MerchantStore cannot be null");

		try {
			return userRepository.findByGroupsAndStore(groups, store.getId());
		} catch (Exception e) {
			throw new ServiceException(e);
		}
	}
