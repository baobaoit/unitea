...
public interface CustomerRepository extends JpaRepository<Customer, Long>, CustomerRepositoryCustom {
	...
	@Query("select c from Customer c "
			...
			+ "left join fetch cb.country "
			+ "left join fetch cb.zone "
			+ "where c.emailAddress = ?1 and cm.id = ?2")
	Customer findByEmailAddress(String emailAddress, int storeId);
	...
	@Query("select count(c) from Customer c left join c.merchantStore cm where cm <> 1")
	int countAll();

	@Query("select count(c) from Customer c left join c.merchantStore cm where cm <> 1 and c.auditSection.dateCreated >= ?1 and c.auditSection.dateCreated <= ?2")
	int countAllInDay(Date fromDate, Date toDate);
}
