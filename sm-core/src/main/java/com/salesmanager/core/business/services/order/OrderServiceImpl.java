...
import com.salesmanager.core.business.services.exception.InsufficientStockException;
...
public class OrderServiceImpl extends SalesManagerEntityServiceImpl<Long, Order> implements OrderService {
    ...
    @Inject
    private ConvenienceFeeService convenienceFeeService;
        ...
        for(OrderProduct orderProduct : products) {
            ...
            for(ProductAvailability availability : p.getAvailabilities()) {
                ...
                if(qty < orderProduct.getProductQuantity()) {
                    ...
                    throw new InsufficientStockException("Product has insufficient stock to provide");
                }
    ...
    private OrderTotalSummary caculateOrder(OrderSummary summary, Customer customer, CustomerTip customerTip,
                ...
                if(otherPrices!=null) {
                    for(FinalPrice price : otherPrices) {
                        if(price.isDefaultPrice()) {
                            ...
                            if(OrderValueType.ONE_TIME.name().equals(price.getProductPrice().getProductPriceType().name())) {
        ...
        //tip
		List<com.salesmanager.core.model.tip.OTTipRate> tipRates = tipService.calculateTip(totalSummary, customerTip, store, language);
        if (tipRates != null && tipRates.size() > 0) {
        	BigDecimal defaultTipRateValue = tipRates.stream()
					.filter(com.salesmanager.core.model.tip.OTTipRate::isActivated)
					.map(com.salesmanager.core.model.tip.OTTipRate::getValue)
					.findFirst().orElse(BigDecimal.ZERO);

        	OrderTotal tipLine = new OrderTotal();
        	tipLine.setModule(Constants.OT_CUSTOMER_TIP_MODULE_CODE);
        	tipLine.setOrderTotalType(OrderTotalType.TIP);
        	tipLine.setOrderTotalCode("order.total.tip");
        	tipLine.setTitle(Constants.OT_CUSTOMER_TIP_MODULE_CODE);
        	tipLine.setText("Tip");
        	tipLine.setSortOrder(300);
        	tipLine.setValue(defaultTipRateValue);
        	orderTotals.add(tipLine);

        	grandTotal = grandTotal.add(defaultTipRateValue);
        	totalSummary.setTipRates(tipRates);
		}

        // Convenience fee
        BigDecimal convenienceFee = convenienceFeeService.getConvenienceFee(store);
        if (convenienceFee != null) {
          OrderTotal convenienceFeeLine = new OrderTotal();
          convenienceFeeLine.setModule(Constants.OT_CONVENIENCE_FEE_MODULE_CODE);
          convenienceFeeLine.setOrderTotalType(OrderTotalType.CONVENIENCE_FEE);
          convenienceFeeLine.setOrderTotalCode("order.total.convenience.fee");
          convenienceFeeLine.setTitle(Constants.OT_CONVENIENCE_FEE_MODULE_CODE);
          convenienceFeeLine.setText("Convenience fee");
          convenienceFeeLine.setSortOrder(301);
          convenienceFeeLine.setValue(convenienceFee);
          orderTotals.add(convenienceFeeLine);

          grandTotal = grandTotal.add(convenienceFee);
        }
    ...
    @Override
    public OrderTotalSummary caculateOrderTotal(final OrderSummary orderSummary, final Customer customer, final CustomerTip customerTip, final MerchantStore store, final Language language) throws ServiceException {
        ...
        try {
            return caculateOrder(orderSummary, customer, customerTip, store, language);
        } catch (Exception e) {
    ...
    @Override
    public OrderTotalSummary caculateOrderTotal(final OrderSummary orderSummary, CustomerTip customerTip, final MerchantStore store, final Language language) throws ServiceException {
        Validate.notNull(orderSummary,"Order summary cannot be null");
        Validate.notNull(orderSummary.getProducts(),"Order summary.products cannot be null");
        Validate.notNull(store,"MerchantStore cannot be null");

        try {
            return caculateOrder(orderSummary, null, customerTip, store, language);
        } catch (Exception e) {
            throw new ServiceException(e);
        }
    ...
    private OrderTotalSummary caculateShoppingCart( ShoppingCart shoppingCart, final Customer customer, final MerchantStore store, final Language language) throws Exception {
    	...
    	return caculateOrder(orderSummary, customer, null, store, language);
	...
	public BigDecimal getTotalTips(MerchantStore store) {
		return tipService.getTotalTips(store);
	}

	@Override
	public BigDecimal getTotalTipsInRangeDate(Date startDate, Date endDate, MerchantStore store) {
		return tipService.getTotalTipsInRangeDate(startDate, endDate, store);
	}
