...
public class CustomerFacadeImpl implements CustomerFacade {
  ...
  private final static String IS_REQUIRED_MESSAGE_SUFFIX = "is/are required";
  ...
  @Override
  public boolean checkIfUserEmailExists(String emailAddress, MerchantStore store) {
    if (StringUtils.isNotBlank(emailAddress) && store != null) {
      Customer customer = customerService.getByEmail(emailAddress, store.getId());
      if (customer != null) {
        LOG.info("Customer with emailAddress {} already exists for store {} ", emailAddress,
                store.getStorename());
        return true;
      }

      LOG.info("No customer found with emailAddress {} for store {} ", emailAddress, store.getStorename());
      return false;

    }
    LOG.info("Either emailAddress is empty or we have not found any value for store");
    return false;
  }
  ...
  public Customer getCustomerModel(final PersistableCustomer customer,
    ...
    Customer customerModel = customerService.getByEmail(customer.getEmailAddress(), merchantStore.getId());
    if(Objects.isNull(customerModel)) {
      customerModel = customerPopulator.populate(customer, merchantStore, language);
    }
    customerModel.setNick(customer.getEmailAddress());
  ...
  public void updateAddress(Long userId, MerchantStore merchantStore, Address address,
    ...
    customerModel.getAuditSection().setDateModified(new Date());
  ...
  private Customer convertPersistableCustomerToCustomer(PersistableCustomer customer, MerchantStore store) {
    ...
    if (Objects.isNull(cust.getAuditSection().getDateCreated())) {
      cust.getAuditSection().setDateCreated(new Date());
    }
  ...
  public void resetPassword(Customer customer, MerchantStore store, Language language)
    ...
    customer.setPassword(encodedPassword);
    customer.getAuditSection().setDateModified(new Date());
  ...
  private ReadableCustomerList convertCustomerListToReadableCustomerList(
    ...
    if (CollectionUtils.isEmpty(customers)) {
      readableCustomerList.setRecordsTotal(0);
      return readableCustomerList;
    }

    List<ReadableCustomer> readableCustomers = customers.stream()
        .map(customer -> convertCustomerToReadableCustomer(customer, store, language))
        .collect(Collectors.toList());

    final int customerListSize = customers.size();

    readableCustomerList.setCustomers(readableCustomers);
    readableCustomerList.setTotalPages(customerList.getTotalPages());
    readableCustomerList.setNumber(customerListSize);
    readableCustomerList.setRecordsFiltered(customerListSize);
  ...
  @Override
  public void setCustomerPassword(Customer target, String password) {
    target.setPassword(passwordEncoder.encode(password));
  }

  @Override
  public Customer populateBillingAddress(Customer customer, BillingAddress billingAddress) throws ServiceException {
    verifyBillingAddress(billingAddress);
    Billing billing = new Billing();

    billing.setAddress(billingAddress.getAddress());
    billing.setCity(billingAddress.getCity());
    billing.setCountry(countryService.getByCode(billingAddress.getCountry()));
    billing.setFirstName(billingAddress.getFirstName());
    billing.setLastName(billingAddress.getLastName());
    billing.setPostalCode(billingAddress.getPostalCode());
    billing.setZone(zoneService.getByCode(billingAddress.getZone()));
    billing.setTelephone(billingAddress.getPhone());

    if (!billing.equals(customer.getBilling())) {
  ...
  private void verifyBillingAddress(BillingAddress billingAddress) {
    StringBuilder builder = new StringBuilder();
    if (StringUtils.isBlank(billingAddress.getAddress())) {
    ...
    if (StringUtils.isBlank(billingAddress.getZone())) {
      builder.append("zone ");
    }

    if (StringUtils.isBlank(billingAddress.getPhone())) {
      builder.append("phone ");
    }

    if (builder.length() > 0) {
      builder.append(IS_REQUIRED_MESSAGE_SUFFIX);
      throw new MissingFieldException(String.valueOf(builder));
    }
  }
}
