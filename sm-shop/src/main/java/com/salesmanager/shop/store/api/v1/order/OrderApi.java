...
import com.salesmanager.core.model.order.CriteriaOrderField;
import com.salesmanager.core.model.order.orderstatus.OrderStatus;
import com.salesmanager.shop.model.order.OrderStatusInfo;
import com.salesmanager.shop.model.order.v1.BillingAddress;
import com.salesmanager.shop.model.order.v1.notification.PersistableOrderNotification;
import com.salesmanager.shop.store.api.exception.MissingBillingAddressException;
import com.salesmanager.shop.store.api.exception.MissingFieldException;
...
public class OrderApi {
	...
	@Inject
	private OrderNotificationFacade orderNotificationFacade;
	...
	public ReadableOrderList list(
			...
			@RequestParam(value = "orderBy", required = false) CriteriaOrderBy orderBy,
			@RequestParam(value = "field", required = false, defaultValue = "ID") CriteriaOrderField field,
		...
		orderCriteria.setCriteriaOrderByField(field.value());
		orderCriteria.setOrderBy(orderBy);
	...
	public ReadableOrderConfirmation checkout(
		...
		try {
			...
			if (Objects.nonNull(order.getBillingAddress())) {
				customer = customerFacadev1.populateBillingAddress(customer, order.getBillingAddress());
			} else {
				throw new MissingBillingAddressException("billingAddress is required");
			}
			...
			// order notification
			orderNotificationFacade.create(modelOrder);
			
			return orderFacadeV1.orderConfirmation(modelOrder, customer, merchantStore, language);
			

		} catch (MissingBillingAddressException | MissingFieldException e) {
			throw e;
		} catch (Exception e) {
	...
	public ReadableOrderConfirmation checkout(
		...
		try {
			...
			if (customerFacade.checkIfUserEmailExists(order.getCustomer().getEmailAddress(), merchantStore)) {
				customer = customerService.getByEmail(order.getCustomer().getEmailAddress(), merchantStore.getId());
			} else {
				customer = customerFacade.populateCustomerModel(customer, order.getCustomer(), merchantStore, language);
			}

			if (Objects.nonNull(order.getCustomer()) && Objects.nonNull(order.getCustomer().getBilling())) {
				BillingAddress billingAddress = new BillingAddress(order.getCustomer().getBilling());

				customer = customerFacadev1.populateBillingAddress(customer, billingAddress);
			} else {
				throw new MissingBillingAddressException("billingAddress is required");
			}
			...
			if(!StringUtils.isBlank(presistableCustomer.getPassword())) { 
				...
				if(customerFacadev1.checkIfUserExists(customer.getNick(),  merchantStore) && Objects.nonNull(customer.getPassword())) {
					//409 Conflict
					throw new GenericRuntimeException("409", "Customer with email [" + customer.getEmailAddress() + "] is already registered");
				} else {
					customerFacadev1.setCustomerPassword(customer, presistableCustomer.getPassword());
				}
	...
	@RequestMapping(value = {"/private/orders/{id}/status/{status}"}, method = RequestMethod.PUT)
	@ResponseStatus(HttpStatus.OK)
	@ResponseBody
	@ApiImplicitParams({ @ApiImplicitParam(name = "store", dataType = "string", defaultValue = "DEFAULT"),
			@ApiImplicitParam(name = "lang", dataType = "string", defaultValue = "en") })
	public void updateOrderStatus(@PathVariable Long id, @PathVariable OrderStatus status,
								  @RequestBody OrderStatusInfo orderStatusInfo, @ApiIgnore MerchantStore merchantStore) throws Exception {
		String user = authorizationUtils.authenticatedUser();
		...
		orderFacade.updateOrderStatus(id, status, merchantStore, orderStatusInfo);
	}

	/**
	 * Allow customers go to the checkout page when the order time is working hours
	 * and payment method is enable.
	 *
	 * @param merchantStore
	 * @param language
	 * @return
	 */
	@GetMapping("/orders/enable-checkout-processes")
	@ApiImplicitParams({ @ApiImplicitParam(name = "store", dataType = "string", defaultValue = "DEFAULT"),
			@ApiImplicitParam(name = "lang", dataType = "string", defaultValue = "en") })
	public ReadableCheckoutInformation enableCheckoutProcesses(@ApiIgnore MerchantStore merchantStore,
			@ApiIgnore Language language) {

		return checkoutFacade.enableCheckoutProcesses(
				businessTimeConfigurationFacade.getConfiguration(merchantStore, language), merchantStore);
	}

	/**
	 * Notifications about the latest orders.
	 * 
	 * @param merchantStore
	 * @param language
	 * @return
	 */
	@GetMapping("/private/orders/notification/latest")
	@ApiImplicitParams({ @ApiImplicitParam(name = "store", dataType = "string", defaultValue = "DEFAULT"),
			@ApiImplicitParam(name = "lang", dataType = "string", defaultValue = "en") })
	public List<ReadableOrderNotification> getLatestOrderNotification(@ApiIgnore MerchantStore merchantStore,
			@ApiIgnore Language language) {

		return orderNotificationFacade.getLatestOrder(merchantStore, language);
	}

	/**
	 * Client confirms receipt of the latest order notification.
	 * 
	 * @param orderNotification
	 * @param merchantStore
	 * @param language
	 */
	@PostMapping("/private/orders/notification/confirm")
	@ApiImplicitParams({ @ApiImplicitParam(name = "store", dataType = "string", defaultValue = "DEFAULT"),
			@ApiImplicitParam(name = "lang", dataType = "string", defaultValue = "en") })
	public void removeOrderNotifications(@RequestBody PersistableOrderNotification orderNotification,
										 @ApiIgnore MerchantStore merchantStore,
										 @ApiIgnore Language language) {

		orderNotificationFacade.remove(orderNotification, merchantStore, language);
	}
}
