...
public class TaxFacadeImpl implements TaxFacade {
	...
	public ReadableTaxRate taxRate(Long id, MerchantStore store, Language language) {
		...
		TaxRate taxRateModel = taxRateById(id, store, language);
		ReadableTaxRate taxRate = readableTaxRateMapper.convert(taxRateModel, store, language);
		TaxClass taxClassModel = taxRateModel.getTaxClass();
		if (taxClassModel != null) {
			ReadableTaxClass taxClass = readableTaxClassMapper.convert(taxClassModel, store, language);
	...
	public ReadableEntityList<ReadableTaxRate> taxRates(MerchantStore store, Language language) {
		...
		try {
			...
			List<ReadableTaxRate> readableRates = rates.stream().map(r -> taxRate(r.getId(), store, language)).collect(Collectors.toList());
