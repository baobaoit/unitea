...
public class OrderFacadeImpl implements OrderFacade {
	...
	private OrderTotalSummary calculateOrderTotal(MerchantStore store, Customer customer,
		...
		if (order instanceof ShopOrder) {
			...
			orderTotalSummary = orderService.caculateOrderTotal(summary, customer, null, store, language);
		} else {
	...
	public Order processOrder(com.salesmanager.shop.model.order.v1.PersistableOrder order, Customer customer,
		...
		try {
			...
			com.salesmanager.core.business.services.model.CustomerTip customerTip = null;
			com.salesmanager.shop.model.tip.PersistableCustomerTip persistableCustomerTip = order.getCustomerTip();
			if (persistableCustomerTip != null) {
				customerTip = new com.salesmanager.core.business.services.model.CustomerTip();
				customerTip.setTipAmount(persistableCustomerTip.getTipAmount());
				customerTip.setType(com.salesmanager.core.business.services.model.CustomerTipType.valueOf(
						persistableCustomerTip.getType().name()));
			}

			orderTotalSummary = orderService.caculateOrderTotal(orderSummary, customer, customerTip, store, language);
	...
	@Override
	public void updateOrderStatus(Long orderId, OrderStatus status, MerchantStore store, OrderStatusInfo orderStatusInfo) throws ServiceException {
		...
		if (order == null) {
			throw new ResourceNotFoundException(
					"Order with id [" + orderId + "] does not exist for merchant [" + store.getCode() + "]");
		}
		...
		if (!status.equals(lastStatus)) {
			...
			if (Objects.nonNull(orderStatusInfo) && StringUtils.isNotBlank(orderStatusInfo.getMessage())) {
				orderStatusHistory.setComments(orderStatusInfo.getMessage());
			} else {
				orderStatusHistory.setComments(messages.getMessage("email.order.status.changed",
						new String[] {order.getStatus().name(), status.name()}, null));
			}
			...
			if (lastStatus.equals(OrderStatus.PROCESSED) && status.equals(OrderStatus.DELIVERED)) {
				sendUpdateOrderStatusEmail(order, orderStatusHistory, store);
			}

		}
	}

	private void sendUpdateOrderStatusEmail(Order order, OrderStatusHistory orderStatusHistory, MerchantStore store) {
		Customer customer = customerService.getById(order.getCustomerId());

		Locale customerLocale = LocaleUtils.getLocale(customer.getDefaultLanguage());

		emailTemplatesUtils.sendUpdateOrderStatusEmail(customer, order, orderStatusHistory, store, customerLocale, "");
	}
