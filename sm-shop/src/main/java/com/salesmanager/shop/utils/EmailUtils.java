...
public class EmailUtils {
	...
	public Map<String, String> createEmailObjectsMap(String contextPath, MerchantStore store, LabelUtils messages, Locale locale){
		...
		if(store.getStoreLogo()!=null) {
			//TODO revise
			StringBuilder logoPath = new StringBuilder("<img src='");
			logoPath.append(imageUtils.buildStoreLogoFilePath(store)).append("' style='max-width:85px;'>");
	...
	public Map<String, String> createEmailObjectsMap(MerchantStore store, LabelUtils messages, Locale locale) {
		return createEmailObjectsMap(null, store, messages, locale);
	}
