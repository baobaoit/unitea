...
public class ReadableOrderPopulator extends
	...
	public ReadableOrder populate(Order source, ReadableOrder target,
		...
		java.util.Set<OrderStatusHistory> orderStatusHistory = source.getOrderHistory();
		if (orderStatusHistory != null && !orderStatusHistory.isEmpty()) {
			orderStatusHistory.stream()
					.filter(osh -> java.util.Arrays.asList(OrderStatus.PROCESSED, OrderStatus.ORDERED).contains(osh.getStatus()))
					.min(Comparator.comparing(OrderStatusHistory::getDateAdded))
					.map(OrderStatusHistory::getDateAdded)
					.map(com.salesmanager.shop.utils.DateUtil::formatInvoiceDateTime)
					.ifPresent(target::setInvoiceDatePurchased);
		}
