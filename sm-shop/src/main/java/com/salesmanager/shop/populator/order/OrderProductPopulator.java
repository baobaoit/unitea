...
public class OrderProductPopulator extends
	...
	public OrderProduct populate(ShoppingCartItem source, OrderProduct target,
		...
		try {
			...
			Set<ProductDescription> productDescriptions = source.getProduct().getDescriptions();
			Optional<String> description = productDescriptions.stream()
					.filter(productDescription -> language.equals(productDescription.getLanguage()))
					.findFirst()
					.map(ProductDescription::getName);
			target.setProductName(description.orElseGet(() -> productDescriptions.iterator().next().getName()));
