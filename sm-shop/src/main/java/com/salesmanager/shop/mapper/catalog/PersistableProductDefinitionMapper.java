...
public class PersistableProductDefinitionMapper implements Mapper<PersistableProductDefinition, Product> {
	...
	public Product merge(PersistableProductDefinition source, Product destination, MerchantStore store,
		...
		try {
			...
			//TAXCLASS
			if(!StringUtils.isBlank(source.getTaxClassCode())) {
				TaxClass taxClass = taxClassService.getByCode(source.getTaxClassCode(),store);
				if(taxClass == null) {
					throw new ConversionException("Tax Class [" + source.getTaxClassCode() + "] does not exist");
				}
