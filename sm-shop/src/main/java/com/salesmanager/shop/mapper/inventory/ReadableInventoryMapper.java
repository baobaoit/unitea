...
import com.salesmanager.shop.model.catalog.product.inventory.ReadableProductDimensions;
...
public class ReadableInventoryMapper implements Mapper<ProductAvailability, ReadableInventory> {
  ...
  public ReadableInventory merge(ProductAvailability source, ReadableInventory destination,
    ...
    try {
      ...
      if (Objects.nonNull(source.getDimensions())) {
        ProductDimensions productDimentions = source.getDimensions();

        ReadableProductDimensions dimensions = new ReadableProductDimensions();
        dimensions.setHeight(productDimentions.getHeight());
        dimensions.setLength(productDimentions.getLength());
        dimensions.setWeight(productDimentions.getWeight());
        dimensions.setWidth(productDimentions.getWidth());

        destination.setDimensions(dimensions);
      }
...
/*    ReadableMerchantStorePopulator populator = new ReadableMerchantStorePopulator();
  ...
  private List<ReadableProductPrice> prices(ProductAvailability source, MerchantStore store, Language language) throws ConversionException {
    ...
    if (!prices.isEmpty()) {
      setActivateForProductPrice(source.getPrices(), prices);
    }

    return prices;
  }

  private void setActivateForProductPrice(Set<ProductPrice> source, List<ReadableProductPrice> destination) {
    List<ProductPrice> prices = source.stream().filter(ProductPrice::isDefaultPrice)
            .sorted(Comparator.comparing(ProductPrice::getId))
            .collect(Collectors.toList());

    if(!prices.isEmpty()) {
      Long id = prices.get(prices.size() - 1).getId();
      destination.stream().filter(readableProductPrice -> id.equals(readableProductPrice.getId()))
              .findFirst().ifPresent(readableProductPrice -> readableProductPrice.setActivated(true));
    }
