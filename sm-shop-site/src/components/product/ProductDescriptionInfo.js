...
const ProductDescriptionInfo = ({
  ...
  const getPrice = async (tempSelectedOptions) => {
    setLoader(true)
    let action = constant.ACTION.PRODUCT + productID + '/' + constant.ACTION.PRICE + '?store=' + window._env_.APP_MERCHANT;
  ...
  const setQuantityProduct = (action) => {
    let newQuantity;
    if (action === 'up') {
      newQuantity = quantityCount <= product.quantity ? quantityCount + 1 : quantityCount;
    }
    if (action === 'down') {
      newQuantity = quantityCount > 1 ? quantityCount - 1 : 1;
    }
                  ...
                  <div className="pro-details-color-content" style={{ display: 'flex', flexDirection: 'column' }}>
                    {
                      ...
                      option.optionValues.map((value, index) => {
                        return (
                          <div style={{ flexDirection: 'row', display: 'flex', alignItems: 'center', margin: 7 }} key={index}>
        ...
        )}
      {
        ...
        <div className="pro-details-quality">
          <div className="cart-plus-minus">
            <button onClick={() => setQuantityProduct('down')} className="dec qtybutton">-</button>
            <input
              ...
              readOnly
            />
            <button onClick={() => setQuantityProduct('up')} className="inc qtybutton">+</button>
          </div>
        ...
        <div className="pro-details-meta">
          ...
          <ul>
            {product.categories.map((single, key) => {
              return (
                <li key={key}>
                  <Link to={"/category/" + single.description.friendlyUrl} onClick={() => onClickCategory(single.description)}>
