...
const Category = ({ setCategoryID, isLoading, strings, location, defaultStore, currentLanguageCode, categoryID, setLoader, }) => {
    ...
    useEffect(() => {
        if (friendlyUrl) {
            loadCategoryByName(friendlyUrl);
        }
    }, [friendlyUrl]);

    // load category by category name
    const loadCategoryByName = async (categoryName) => {
        setLoader(true);
        setIsLoadingProducts(true);
        let action = `${constant.ACTION.CATEGORY}name/${categoryName}?lang=${currentLanguageCode}`;
        try {
            let response = await WebService.get(action);
            setCategoryIdData(Number(response.id));
        } catch (error) {
            setLoader(false);
    ...
    useEffect(() => {
        if (categoryIdData) {
            setCategoryValue(categoryIdData);
            getProductList(categoryIdData, [], []);
        }
        setSubCategory([]);
        setColor([]);
        setManufacture([]);
            ...
            <Layout headerContainerClass="container-fluid"
                ...
                <div className="breadcrumb-area pt-35 pb-35 bg-gray-3">
                    <div className="container">
                        <div className="breadcrumb-content text-center breadcrumb-content-custom">
                            <Nav.Item>
                                <Nav.Link href="/">Home</Nav.Link>
                            </Nav.Item>
                            {' / '}
                            {!isLoadingData && friendlyUrl && <Nav.Item>
                                <Nav.Link href={friendlyUrl}>{name}</Nav.Link>
                            </Nav.Item>}
                        </div>
                    </div>
                </div>
                    ...
                    <div className="container">
                        {
                            productData.length > 0 ?
                                (<div className="row">
                                    ...
                                    {/* </div> */}
                                    <div className="col-lg-12 order-1 order-lg-12">
                                        {/* shop topbar default */}
                                ...
                                :
                                (
                                    !isLoadingProducts && <div className="col-lg-12">
