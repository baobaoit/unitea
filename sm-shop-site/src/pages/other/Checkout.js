...
const Checkout = ({
  ...
  const [tipRatesType, setTipRatesType] = useState();
  const [tipAmountValue, setTipAmountValue] = useState();
  const [tipRates, setTipRates] = useState([]);
...
// console.log(window._env_.APP_PAYMENT_TYPE);
  ...
  const showErrorMessage = (messageError, allowOutsideClick) => {
    Swal.fire({
      title: 'Oops...',
      html: messageError,
      icon: 'error',
  ...
  const initCheckOrder = async () => {
    setLoader(true);
    const action = constant.ACTION.ORDERS + 'enable-checkout-processes' + '?store=' + defaultStore;
    try {
      let response = await WebService.get(action);
      if (response && !response.enabled) {
        const messageTypes = response.types;
        if (messageTypes && messageTypes.length === 2) {
          const newMessageErrorWithHour = `Please go to nearest store to order. Love see you there.`;
          showErrorMessage(newMessageErrorWithHour);
        } else if (messageTypes && messageTypes.length === 1){
          const typeError = messageTypes[0];
          switch (typeError) {
            case 'BUSINESS_HOURS_CLOSE':
              const newMessageErrorWithHour = `We are very sorry that you are unable to check out your order because the business closed. We will open from ${timeShopWorking.startTime} to ${timeShopWorking.endTime} and love to see you soon.`;
              showErrorMessage(newMessageErrorWithHour);
              break;
            case 'PAYMENT_NOT_SUPPORT':
              const newMessageErrorWithPayment = `We are very sorry that your are unable to check out your order because the payment is under construction. Please go to a nearest store and our very nice and friendly staffs love to see you.`;
              showErrorMessage(newMessageErrorWithPayment);
              break;
            default:
              break;
          }
  ...
  const setDefaultsValue = () => {
    // set country default
    const defaultCountry = merchant.address.country || 'US';
  ...
  const onChangeShipping = async () => {
    let action = constant.ACTION.CART + cartID + '/' + constant.ACTION.SHIPPING + '?store=' + window._env_.APP_MERCHANT;
    let param = {};
  ...
  const shippingQuoteChange = async (quoteID) => {
    let action = `${userData ? constant.ACTION.AUTH : ''}${constant.ACTION.CART}${cartID}/${constant.ACTION.TOTAL}?store=${window._env_.APP_MERCHANT}`;
    ...
    try {
      let response = await WebService.get(action);
      if (response) {
        setTipRates(response.tipRates);

        // set auto load tip rates
        const _tipRates = _.filter(response.tipRates, d => d.activated === true);
        if (_tipRates && _tipRates.length > 0) {
          applyTipRate('PERCENTAGE', _tipRates[0].percentage);
        } else {
  ...
  const onSubmitOrder = async (data, elements, stripe) => {
    ...
    // if (!shopOpenNow) {
    //   addToast('An Unitea shop closed the order, please comeback to order after. Thanks!', { appearance: "error", autoDismiss: true });
    //   return;
    // }
    ...
    setLoader(true);
    // check availiable to add to cart
    const action = constant.ACTION.ORDERS + 'enable-checkout-processes' + '?store=' + defaultStore;
    let response = await WebService.get(action);
    if (response && !response.enabled) {
      setLoader(false);
  ...
  const onPayment = async (data, result) => {
    ...
    let param = {};
    if (userData) {
      action = constant.ACTION.AUTH + constant.ACTION.CART + cartID + '/' + constant.ACTION.CHECKOUT + '?store=' + window._env_.APP_MERCHANT;
      param = {
        ...
        "payment": {
          ...
          "paymentToken": result,
          "amount": (shippingQuote[shippingQuote.length - 1].value).toFixed(2)
        }
      }
    } else {
      action = constant.ACTION.CART + cartID + '/' + constant.ACTION.CHECKOUT + '?store=' + window._env_.APP_MERCHANT;
      let customer = {};
      if (isShipping) {
        customer = {
    ...
    // add tip rates
    if ((tipAmountValue && tipRatesType) || (tipAmountValue === 0 && tipRatesType === 'PERCENTAGE')) {
      param['customerTip'] = {
    ...
    param['customerAgreement'] = data.isAgree;
     
    try {
      const response = await WebService.post(action, param);
      if (response) {
        reset({});
        ref.clear();
  ...
  const onAgreement = async() => {
    let action = constant.ACTION.CONTENT + constant.ACTION.BOXES + constant.ACTION.AGREEMENT + '?store=' + window._env_.APP_MERCHANT;
    try {
      let response = await WebService.get(action);
      if (response) {
  ...
  // TIP RATES
  const onSelectedTip = (data) => {
    const cloneData = _.cloneDeep(tipRates);
    cloneData.map(d => {
      d.activated = d.value === data.value;
      return {...d};
    });
    setTipRates(cloneData);
    setTipAmountCustom(null);
    setIsCollapse(false);
    document.getElementById("tipAmountElement").value = null;
    applyTipRate('PERCENTAGE', data.percentage);
  };

  const applyTipRate = async (type, tipAmount) => {
    setLoader(true);
    let action = ''
    if (userData) {
      action = constant.ACTION.AUTH + constant.ACTION.CART + cartID + '/total?store=' + defaultStore + `&type=${type}&tipAmount=${tipAmount}`;
    } else {
      action = constant.ACTION.CART + cartID + '/total?store=' + defaultStore + `&type=${type}&tipAmount=${tipAmount}`;
    }
    try {
  ...
  const onApplyTipCustom = () => {
    if (tipAmountCustom) {
      ...
      setTipRates(cloneData);
      applyTipRate('CUSTOM', tipAmountCustom);
    }
  };
                        ...
                        <div className="col-lg-12">
                          <div className="billing-select mb-20">
                            ...
                            <Controller
                              ...
                              render={props => {
                                return (
                                  <select disabled={true} onChange={(e) => { props.onChange(e.target.value); getState(e.target.value); onChangeShipping() }} value={props.value}>
                      ...
                      <div className="additional-info-wrap">
                        ...
                        <div className="additional-info">
                          ...
                          <textarea
                            ...
                            name={paymentForm.orderNotes.name}
                            ref={register(paymentForm.orderNotes.validate)}
                            defaultValue={""}
                          />
                        </div>
                      </div>
                    ...
                    <div className="your-order-area">
                      ...
                      <div className="your-order-wrap gray-bg-4">
                        <div className="your-order-product-info">
                              ...
                              {cartItems.products.map((cartItem, key) => {
                                ...
                                return (
                                  <React.Fragment key={key}>
                                    ...
                                    <li className={'product-name'} key={key}>
                                      <span className="order-middle-left name-product" style={{ width: 220 }}>
                                    ...
                                    <li className={'topping topping-free'}>
                                      {
                                        cartItem.cartItemattributes.map((topping, index) => {
                                          if (topping.option.code.includes('OPTION_FREE') && topping.optionValue.name !== 'No') {
                                            return <React.Fragment>
                                              <ul>
                                                  <li key={index}>
                                                    <span className="order-middle-left">
                                                      ({topping.optionValue.name === 'Yes' ? topping.option.name : topping.optionValue.name})
                                                    </span>
                                                  </li>
                                              </ul>
                                            </React.Fragment>;
                                    ...
                                    <li className={'topping'}>
                                      {
                                        cartItem.cartItemattributes.map((topping, index) => {
                                          if (topping.option.code.includes('OPTION_FREE')) {
                                              ...
                                              <ul>
                                                  <li key={index}>
                                                    <span className="order-middle-left" style={{ width: 220 }}>
                                                      {topping.optionValue.name}
                                                    </span>{" "} 
                                                    <span className="order-price">
                                                      {
                                                        topping.optionValue.price
                                                      }
                                                    </span>
                                                  </li>
                                              </ul>
                          ...
                          {/* Tips rates */}
                          {tipRates.length > 0 && <div className="your-order-tip-rates">
                            <label>Leave a tip</label>
                            <ul>
                                {tipRates.map((item, index) => {
                                  return (
                                    <li key={index} className={item.activated ? 'active' : ''} onClick={() => onSelectedTip(item)}>
                                      <span>{item.percentage === 0 ? 'None' : `${item.percentage} %`}</span>
                                      {item.percentage > 0 && <span>{item.total}</span>}
                                    </li>
                                  )
                                })}
                            </ul>
                          ...
                          <div className="card your-order-tip-rates-custom">
                              <div className="card-header" onClick={() => {
                                    setIsCollapse(!isCollapse);
                                }}>
                                <span>Custom Tip Amount</span>
                                <Button variant="link" size="sm" className="collapse-button">
                                  {!isCollapse && <i className="fa fa-chevron-right"></i>}
                                  {isCollapse && <i className="fa fa-chevron-down"></i>}
                                </Button>
                              </div>
                              <div className={isCollapse ? 'card-body' : 'card-body hidden'}>
                                
                                  <div className="row">
                                    <div className="col-lg-12 col-md-12">
                                      <div className="billing-info">
                                        <input type="number" min="0" name="tipAmountCustom" id="tipAmountElement" onChange={($event) => {
                                          if ($event && $event.target) {
                                            const newValue = $event.target.value;
                                            if (newValue && Number(newValue) > 0) {
                                              setTipAmountCustom(newValue);
                                            } else {
                                              setTipAmountCustom(null);
                                            }
                                          }
                                  ...
                                  <div className="row">
                                    <div className="col-lg-12 col-md-12 text-right">
                                      <Button variant={!tipAmountCustom ? 'outline-success' : 'success'} size="sm" disabled={!tipAmountCustom} 
                                        onClick={() => onApplyTipCustom()} className="save-amount-button">
                                          Save
                                      </Button>
                                    </div>
                                  </div>
                          ...
                          <div className="your-order-sub-total">
                            {
                              ...
                              shippingQuote.map((quote, i) => {
                                ...
                                if ('tax' === quote.module) {
                                  quoteTitle = `Tax Rate (${quote.taxRate}%)`;
                                }
                                return (
                                  quote.title !== 'Total' &&
                                  <ul className="mb-20" key={i}>
                                    <li className="order-total">{quoteTitle}</li>
                                    <li>
                                      {quote.total}
                                    </li>
                                  </ul>)
...
const mapStateToProps = state => {
  return {
    ...
    shopOpenNow: state.merchantData.shopOpenNow,
    timeShopWorking: state.merchantData.timeShopWorking,
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    getShippingCountry: (code) => {
      dispatch(getShippingCountry(code));
    },
