...
const Cart = ({
  ...
  const deleteAllFromCart = () => {
    deleteFromCartOnServer(cartItems.code, defaultStore, addToast);
    //go to home page
    // history.push('/');
  }
                    ...
                    <div className="table-content table-responsive cart-table-content">
                      <table>
                        <thead>
                          <tr>
                            <th>{strings["Image"]}</th>
                            <th className={'header-product-name'}>{strings["Product Name"]}</th>
                        ...
                        <tbody>
                          {cartItems.products.map((cartItem, key) => {
                            ...
                            const { cartItemattributes } = cartItem;
                            ...
                            return (
                              <tr key={key}>
                                ...
                                <td className="product-name">
                                  ...
                                  <div className={'topping-free'}>
                                      {
                                        cartItemattributes.map((topping, index) => {
                                  ...
                                  <div className="pro-details-topping-content" style={{ display: 'flex' }}>
                                      {
                                        cartItemattributes.map((topping, key) => {
                                          if (topping.option.code.includes('OPTION_FREE')) {
                                            return <React.Fragment></React.Fragment>;
                                          }
                                          return (
                                            <div style={{ flexDirection: 'column ', display: 'flex', alignItems: 'center', marginRight: 7, marginBottom: 7 }} key={key}>
                                              <label className={`pro-details-size-content--single`} key={key}  >
                                                <span className="size-name">{topping.optionValue.name} {topping.optionValue.price && '(' + topping.optionValue.price + ')'}</span>
                                              </label>
                                            </div>
                                          );
...
Cart.propTypes = {
  ...
  deleteFromCartOnServer: PropTypes.func
};
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    decreaseQuantity: (cartItems, key, item, addToast, cartId, quantityCount, defaultStore) => {
      ...
      let attributes = null;
      const cartItemattributes = item.cartItemattributes;
      if (isValidObject(cartItemattributes) && cartItemattributes.length > 0) {
        attributes = cartItemattributes.map(attr => { 
          return { 
            id: attr.productOptionValueId 
          }
        });
      }

      dispatch(addToCart(item, addToast, cartId, quantityCount, defaultStore, undefined, attributes || undefined));
    },
    increaseQuantity: (item, cartId, quantityCount, addToast) => {
      ...
      dispatch(addToCart(item, addToast, cartId, quantityCount, '', undefined, attributes || undefined));
    },
    getValue: () => {
      return 5;
    },
    deleteFromCart: (cartId, item, defaultStore, addToast) => {
      ...
      const cartItemattributes = item.cartItemattributes;
      if (isValidObject(cartItemattributes) && cartItemattributes.length > 0) {
        attributes = cartItemattributes.map(attr => attr.productOptionValueId).join();
      }
      
      dispatch(deleteFromCart(cartId, item, attributes, defaultStore, addToast));
    },
    getState: (code) => {
      dispatch(getState(code));
    },
    deleteFromCartOnServer: (code, defaultStore, addToast) => {
      dispatch(deleteFromCartOnServer(code, defaultStore, addToast));
    },
  };
};
