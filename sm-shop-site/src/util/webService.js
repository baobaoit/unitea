...
const STORE = window._env_.APP_MERCHANT;
const EXCEPTION_ADD_STORE = 'content/boxes/headerMessage';
...
axios.interceptors.request.use(async (config) => {
    ...
    if (currentURL && !currentURL.includes(EXCEPTION_ADD_STORE) && !currentURL.includes(STORE) && !currentURL.includes('store=')) {
        if (currentURL.includes('?')) {
            config.url += `&store=${STORE}`;
        }
        if (!currentURL.includes('?')) {
            config.url += `?store=${STORE}`;
        }
