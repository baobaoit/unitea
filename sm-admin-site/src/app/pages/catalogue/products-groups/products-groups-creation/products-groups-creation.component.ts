...
export class ProductsGroupsCreationComponent implements OnInit {
  ...
  constructor(
    ...
    this.optionService.getListOfOptions({ store: environment.merchantStore }).subscribe(res => {
      res.options.forEach((option) => {
        this.options.push({ value: option.code, label: option.code });
      });
    });
    this.optionValuesService.getListOfOptionValues({ store: environment.merchantStore }).subscribe(res => {
