...
export class AddVariationsComponent implements OnInit {
  ...
  getOption() {
    this.productOption = []
    this.loading = true
    this.optionService.getListOfOptions({ store: this.storageService.getMerchant() })
      .subscribe((res) => {
  ...
  getOptionValue() {
    ...
    this.optionValuesService.getListOfOptionValues({ store: this.storageService.getMerchant() })
      .subscribe(res => {
        // console.log(res);
