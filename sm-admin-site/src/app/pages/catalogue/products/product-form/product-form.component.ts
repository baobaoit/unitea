...
export class ProductFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const taxClass$ = this.productService.getTaxClass(payload);

    forkJoin([manufacture$, types$, config$, taxClass$])
      .subscribe(([manufacturers, productTypes, languages, taxClassResponse]) => {
        ...
        // tax class
        taxClassResponse.items.forEach((option) => {
          this.taxClasses.push({ value: option.code, label: option.code });
        });
  ...
  fillForm() {
    ...
    this.form.patchValue({
      ...
      // placementOrder: [0, [Validators.required]],  // ???
      taxClass: this.product.taxClass === null ? '' : this.product.taxClass.code,
  ...
  **/
  ...
  save() {
    ...
    this.loading = true;
    const productObject = _.cloneDeep(this.form.value);
    ...
    if (tmpObj.name === '' || tmpObj.friendlyUrl === '' || productObject.identifier === '' || productObject.manufacturer === '' || tmpObj.title === '') {
      ...
      // tax class
      productObject.taxClassCode = productObject.taxClass;
      delete productObject.taxClass;
