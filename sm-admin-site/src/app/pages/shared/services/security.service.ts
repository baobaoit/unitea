...
export class SecurityService {
  ...
  isRetailAdmin(): boolean {
    ...
    if(
      (JSON.parse(localStorage.getItem('roles'))).isSuperadmin || (JSON.parse(localStorage.getItem('roles'))).isAdminRetail || (JSON.parse(localStorage.getItem('roles'))).isAdminCatalogue
    ) {
      return true;
    } else  {
      return false;
