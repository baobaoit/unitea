...
export class StoreService {
  ...
  loadBusinessHours(storeCode: string): Observable<any> {
    return this.crudService.get(`/v1/private/configurations/business-time?store=${storeCode}`);
  }

  saveBusinessHours(storeCode: string, data: any): Observable<any> {
    const payload = {
      businessSchedules: data
    };
    return this.crudService.put(`/v1/private/configurations/business-time?store=${storeCode}`, payload);
  }

  loadStoreCoordinate(storeCode: string): Observable<any> {
    return this.crudService.get(`/v1/private/configurations/coordinate?store=${storeCode}`);
  }

  saveStoreCoordinate(storeCode: string, data: any): Observable<any> {
    const payload = {
      coordinate: data
    };
    return this.crudService.put(`/v1/private/configurations/coordinate?store=${storeCode}`, payload);
  }

  loadConvenienceFee(storeCode: string): Observable<any> {
    return this.crudService.get(`/v1/private/configurations/convenience-fee?store=${storeCode}`);
  }

  saveConvenienceFee(storeCode: string, data: any): Observable<any> {
    const payload = {
      convenienceFee: data.convenienceFee,
      enabled: data.enabled
    };
    return this.crudService.put(`/v1/private/configurations/convenience-fee?store=${storeCode}`, payload);
  }
