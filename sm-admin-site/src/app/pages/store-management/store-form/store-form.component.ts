...
export class StoreFormComponent implements OnInit {
  ...
  constructor(
    ...
    forkJoin(
      ...
      .subscribe(([countries, currencies, measures, stores, languages]) => {
        ...
        this.form.controls['storeApiUrl'].disable();
        if (this.securityService.isSuperAdmin()) {
          ...
          this.form.controls['storeApiUrl'].enable();
        }
  ...
  private createForm() {
    this.form = this.fb.group({
      ...
      contactEmail: ['', [Validators.required, Validators.pattern(validators.emailPattern)]],
  ...
  fillForm() {
    ...
    this.supportedLanguagesSelected.push('en');
    ...
    this.form.patchValue({
      ...
      storeApiUrl: this.store.storeApiUrl || '',
    });
  ...
  get storeApiUrl() {
    return this.form.get('storeApiUrl');
  }
  ...
  save() {
    ...
    storeObj.supportedLanguages = ['en'];
  ...
  canRemove() {
    return this.store.id && ((this.roles.isSuperadmin && this.establishmentType === 'RETAILER')
      || (this.roles.isSuperadmin && this.establishmentType === 'STORE')) && this.store.code !== environment.merchantStore;
  }
  onClickRoute(link) {
