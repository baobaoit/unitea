...
export class UserFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    let stores$ = of([
      { 
        code: this._user?.merchant 
      }
    ]);
    if (this.canGetListOfMerchantStore()) {
      stores$ = this.storeService.getListOfMerchantStoreNames({ 'store': this.store });
    }
  ...
  private canGetListOfMerchantStore(): boolean {
    return this.roles.isSuperadmin || this.roles.isAdmin || this.roles.isAdminRetail;
  }
